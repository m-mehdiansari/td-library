...
function tdbot.setChatDescription(chat_id, description, callback, data)
  assert (tdbot_function ({
...
function tdbot.pinChatMessage(chat_id, message_id, disable_notification, callback, data)
  assert (tdbot_function ({
    ["@type"] = 'pinChatMessage',
    chat_id = chat_id,
    message_id = message_id,
...
function tdbot.unpinChatMessage(chat_id, callback, data)
  assert (tdbot_function ({
...
function tdbot.addProxy(type, server, port, hash, enable, callback, data)
	local input = {}
	...
	elseif type == "Socks5" or type == "Http" then
		input = {
		  ["@type"] = "proxyType"..type,
...
function tdbot.removeProxy(proxy_id, callback, data)
	assert(tdbot_function({
		["@type"] = "removeProxy",
		proxy_id=proxy_id
	}, callback or dl_cb, data))
...
function tdbot.enableProxy(proxy_id, callback, data)
	assert(tdbot_function({
		["@type"] = "enableProxy",
		proxy_id=proxy_id
	}, callback or dl_cb, data))
...
function tdbot.pingProxy(proxy_id, callback, data) 
	assert(tdbot_function({
		["@type"] = "pingProxy",
		proxy_id=proxy_id
	}, callback or dl_cb, data))
...
function tdbot.disableProxy(callback, data) 
	assert(tdbot_function({
		["@type"] = "disableProxy",
	}, callback or dl_cb, data))
end

function tdbot.getProxies(callback, data) 
	assert(tdbot_function({
		["@type"] = "getProxies",
	}, callback or dl_cb, data))
end
